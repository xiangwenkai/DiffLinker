...
from src.egnn import Dynamics, Pre_Dynamics
...
class Pre_EDM(torch.nn.Module):
    def __init__(
            self,
            dynamics: Union[Pre_Dynamics],
            in_node_nf: int,
            n_dims: int,
            timesteps: int = 1000,
            noise_schedule='learned',
    ...
    def forward(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context=None, pre_info=None):
        # Normalization and concatenation
        x, h = self.normalize(x, h)
        xh = torch.cat([x, h], dim=2)
    ...
    @torch.no_grad()
    def sample_chain(self, x, h, node_mask, fragment_mask, linker_mask, edge_mask, context, keep_frames=None, pre_info=None):
        n_samples = x.size(0)
        n_nodes = x.size(1)
    ...
    def sample_p_zs_given_zt_only_linker(self, s, t, z_t, node_mask, fragment_mask, linker_mask, edge_mask, context, pre_info=None):
        """Samples from zs ~ p(zs | zt). Only used during sampling. Samples only linker features and coords"""
        gamma_s = self.gamma(s)
        gamma_t = self.gamma(t)
