    ...
    for pocket in pockets:
        ...
        for sdf_file, pdb_file in zip(sdf_files, pdb_files):
            ...
            for atom_index, nci_type in zip(atom_indexes, nci_types):
                ...
                # ======================== fragment_mask ========================
                fragment_only_mask_ = torch.cat([
                    torch.ones_like(frag_charges),
                    torch.zeros_like(pocket_charges),
                    torch.zeros_like(link_charges)
                ])
                pocket_mask_ = torch.cat([
                    torch.zeros_like(frag_charges),
                    torch.ones_like(pocket_charges),
                    torch.zeros_like(link_charges)
                ])
                fragment_mask_ = torch.cat([
                    torch.ones_like(frag_charges),
                    torch.ones_like(pocket_charges),
                    torch.zeros_like(link_charges)
                ])
                # ======================== linker_mask ========================
                linker_mask_ = torch.cat([
                    torch.zeros_like(frag_charges),
                    torch.zeros_like(pocket_charges),
